...
def search_wolframAlpha(query = ''):
    ...
    # Query resolved 
        ...
        # if it's primary or has title of result or definition, then it is the official resutl
        if (('result') in pod1['@title'].lower()) or (pod1.get('@primary', 'false') == 'true') or ('definition' in pod1['@title'].lower()):
            # Get the result
            result = listOrDict(pod1['subpod'])
            # Remove the bracketed section of result
            return result.split('(')[0]
        else:
            question = listOrDict(pod0['subpod'])
            # Remove the bracketed section of question
...
if __name__ == '__main__':
    speak('All systems nominal. Welcome back Ethan. ') #said during startup
    ...
    while True:
        ...
        if query[0] == activationWord:
            ...
            if query[0] == 'exit':
                speak('Goodbye Ethan. System shutting down.')
                break
